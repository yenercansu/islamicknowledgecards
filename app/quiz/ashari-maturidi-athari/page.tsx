...
import quizData from "@/data/quizzes/ashari-maturidi-athari.json"
...
export default function AshariMaturidiAthariQuiz() {
  const router = useRouter()
  const [currentQuestion, setCurrentQuestion] = useState(0)
  const [selectedAnswers, setSelectedAnswers] = useState<{ [key: number]: string }>({})
  const [scores, setScores] = useState<Scores>({ Ashari: 0, Maturidi: 0, Athari: 0 })
  ...
  const handleSubmit = () => {
    const newScores: Scores = { Ashari: 0, Maturidi: 0, Athari: 0 }
    ...
    quizData.questions.forEach((q, index) => {
      ...
      if (selectedOptionId) {
        ...
        if (option) {
          newScores.Ashari += option.scores.Ashari
          newScores.Maturidi += option.scores.Maturidi
          newScores.Athari += option.scores.Athari
        }
      }
    })
  ...
  const handleRetake = () => {
    ...
    setSelectedAnswers({})
    setScores({ Ashari: 0, Maturidi: 0, Athari: 0 })
    setShowResults(false)
  }

  if (showResults) {
    const total = scores.Ashari + scores.Maturidi + scores.Athari
    const percentages = {
      Ashari: total > 0 ? Math.round((scores.Ashari / total) * 100) : 0,
      Maturidi: total > 0 ? Math.round((scores.Maturidi / total) * 100) : 0,
      Athari: total > 0 ? Math.round((scores.Athari / total) * 100) : 0,
    }

    const sortedSchools = Object.entries(percentages).sort(([, a], [, b]) => b - a)
    const topSchool = sortedSchools[0][0] as keyof Scores
    const topResult = quizData.results[topSchool]
        ...
        <div className="max-w-3xl mx-auto">
          <div className="rounded-3xl bg-white p-6 sm:p-8 border-2 border-teal-200 shadow-lg font-sans">
            ...
            <div className="space-y-6 mb-8">
              {sortedSchools.map(([school, percentage]) => (
                <div key={school}>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-lg font-semibold text-gray-900">{school}</span>
            ...
            <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-2xl p-6 mb-6">
              <h3 className="text-lg font-bold text-gray-900 mb-2">Your Theological Orientation</h3>
              <p className="text-gray-700">
                Based on your answers, you align most closely with the{" "}
                <span className="font-bold">{topResult.name}</span> school ({sortedSchools[0][1]}%).{" "}
