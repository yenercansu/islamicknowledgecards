...
export default function LoginPage() {
  ...
  const canSubmitSignin = useMemo(
    () => siEmail.trim().length > 3 && siPass.trim().length >= 6,
    [siEmail, siPass]
  );
  const canSubmitSignup = useMemo(
    () =>
      suName.trim().length > 1 &&
      suEmail.trim().length > 3 &&
      suPass.trim().length >= 6,
    ...
    <div className="min-h-screen bg-gradient-to-b from-teal-50 to-white relative">
      ...
      <div className="absolute left-4 top-4 z-10">
        <button
          ...
          className="inline-flex h-10 w-10 items-center justify-center rounded-full
               border border-neutral-200 bg-white/90 text-neutral-700 shadow-sm
               backdrop-blur hover:bg-white hover:text-neutral-900
               active:scale-95 transition
               focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-400
               dark:bg-neutral-900/80 dark:border-neutral-800 dark:text-neutral-200 dark:hover:text-white"
        >
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="w-full max-w-[28rem]">
          <div className="rounded-2xl shadow-lg bg-white p-8 text-neutral-900">
            ...
            {mode === "landing" && (
              <div className="space-y-3">
                <button
                  ...
                  className={`w-full rounded-xl border px-4 py-3 font-medium flex items-center justify-center gap-2 ${
                    authDisabled
                ...
                >
                  <GoogleIcon />
                  <span className="text-current">Sign in with Google</span>
                </button>

                <button
                  type="button"
                  onClick={() => setMode("signin")}
                  className="w-full rounded-xl border px-4 py-3 font-medium bg-white text-neutral-900 hover:bg-neutral-50"
                >
                  Sign in
                </button>

                <div className="text-center text-sm text-neutral-700 pt-2">
                  Not a user yet?{" "}
                  <button
                    type="button"
            ...
            {mode === "signin" && (
              <div className="space-y-4">
                <div className="flex items-center justify-between mb-1">
                  <Link
                    href="/"
                    className="flex items-center gap-2 text-sm text-neutral-700 hover:text-teal-700 transition-colors"
                  >
                ...
                <label className="block">
                  <span className="text-sm text-neutral-800">Password</span>
                  <div className="mt-1 relative">
                    <input
                      type={showSiPass ? "text" : "password"}
                      className="w-full rounded-xl border px-3 py-2 pr-12 outline-none focus:ring-2 focus:ring-teal-200"
                      value={siPass}
                    ...
                    <button
                      type="button"
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-neutral-700 hover:text-neutral-900"
                ...
                <button
                  ...
                  className="w-full rounded-xl border px-4 py-3 font-medium bg-white text-neutral-900 hover:bg-neutral-50 disabled:opacity-50"
                >
                ...
                {msg && <p className="text-center text-xs text-neutral-700 mt-2">{msg}</p>}
              </div>
            )}
            ...
            {mode === "signup" && (
              <div className="space-y-4">
                ...
                <label className="block">
                  <span className="text-sm text-neutral-800">Create password</span>
                  <div className="mt-1 relative">
                    <input
                ...
                {msg && <p className="text-center text-xs text-neutral-700 mt-3">{msg}</p>}
              </div>
            )}
          </div>
        </div>
